...
for (let i = 0; i < chairsHall.length; i++) {
  chairsHall[i].addEventListener('click', function () {
    ...
    valueRows.oninput = function () {
      ...
      if (valueRows.value < 3) {valueRows.value = 3};
        ...
        chairsHall.forEach(chairs => {
          if (chairs.checked) {
            ...
            $.ajax({
              ...
              success: function (data) {
                // alert ('Конфигурация зала успешно обновлена')
                alert (data.success);
                window.location.href = 'admin';
              },
              error: function () {
                // alert ('Ошибка! Поля "Рядов" и "Мест" должны содержать цифры')
              }
            })
          }
        })
